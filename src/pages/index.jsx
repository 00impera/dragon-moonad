...
const GOLD_ADDRESS    = "0x86eFC159669969F4066153F06452e1aD55570E5d";
const STAKING_ADDRESS = "0xA65c68e2486c806E9C47d41391Dd3e42568bf4e8";
...
function Logo({ size = 48, src }) {
  ...
  const s = size;
  if (err) return (
    <div style={{ width: s, height: s, borderRadius: "50%", flexShrink: 0, background: "radial-gradient(circle,#050A0E,#001a00)", border: `${s > 80 ? 3 : 2}px solid #FFD700`, boxShadow: "0 0 24px rgba(255,215,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: s > 80 ? 44 : 20 }}>🐲</div>
  );
  return (
    <img src={src || COIN_LOGO} alt="Dragon" width={s} height={s}
      style={{ borderRadius: "50%", flexShrink: 0, display: "block", objectFit: "cover", border: `${s > 80 ? 3 : 2}px solid #FFD700`, boxShadow: s > 80 ? "0 0 40px rgba(255,215,0,0.7),0 0 80px rgba(57,255,20,0.3)" : "0 0 16px rgba(255,215,0,0.6)" }}
      onError={() => setErr(true)}
    />
  );
}
  ...
  return (
    <div style={{ position: "relative", display: "inline-block" }}>
      ...
      {[{ i: -8, d: "0s" }, { i: -16, d: "0.5s" }].map((r, k) => (
        <div key={k} style={{ position: "absolute", inset: r.i, borderRadius: "50%", border: "1.5px solid rgba(255,215,0,0.5)", animation: `pingRing 2s ease-out infinite ${r.d}`, pointerEvents: "none" }} />
      ))}
    </div>
  );
}
...
function ConnectPrompt({ msg = "access this feature" }) {
  return (
    <div className="connect-prompt">
      ...
      <p style={{ color: "rgba(255,229,102,0.6)", marginBottom: 20, fontSize: 13, lineHeight: 1.7 }}>Connect your wallet to {msg}</p>
  ...
  return (
    <>
      <div className="balance-display">
        ...
        <div style={{ borderTop: "1px solid rgba(255,215,0,0.12)", paddingTop: 10, marginTop: 10 }}>
          <div style={{ fontSize: 13, color: "rgba(255,229,102,0.6)" }}>GOLD: <strong style={{ color: "#FFD700" }}>{fmt(gBal)} GOLD</strong></div>
          <div style={{ fontSize: 10, color: "rgba(255,215,0,0.3)", marginTop: 3, fontFamily: "monospace" }}>Supply: {fmt(sup)} DRAGON</div>
        </div>
      ...
      <div style={{ marginTop: 14 }}>
        {!showBuy
          ...
          : <><button className="btn-outline" style={{ marginBottom: 10 }} onClick={() => setBuy(false)}>CLOSE</button>
              <div style={{ borderRadius: 12, overflow: "hidden", border: "1px solid rgba(255,215,0,0.3)" }}>
...
function StakeTab({ account }) {
  ...
  if (!account) return <ConnectPrompt msg="stake MON and earn GOLD + DRAGON" />;
  return (
    <>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 18 }}>
        {[
          ...
          ["PENDING DRAGON", fmt(pend) + " DRAGON"],
        ].map(([l, v]) => (
          <div key={l} style={{ background: "rgba(20,15,0,0.9)", border: "1px solid rgba(255,215,0,0.18)", borderRadius: 10, padding: "12px 14px", textAlign: "center" }}>
            <div style={{ fontSize: 8, letterSpacing: 2, color: "rgba(255,215,0,0.38)", fontFamily: "'Cinzel',serif", textTransform: "uppercase" }}>{l}</div>
            <div style={{ fontFamily: "'Cinzel',serif", fontSize: 12, fontWeight: 700, color: "#FFD700", marginTop: 4 }}>{v}</div>
          </div>
        ))}
      </div>

      <div style={{ background: "rgba(57,255,20,0.05)", border: "1px solid rgba(57,255,20,0.18)", borderRadius: 10, padding: "11px 14px", marginBottom: 18, fontSize: 12, color: "rgba(255,229,102,0.7)", lineHeight: 1.9 }}>
        <strong style={{ color: "#39FF14" }}>Rate:</strong> 1 GOLD + 1 DRAGON per MON per day &nbsp;|&nbsp;
        <strong style={{ color: "#ff6ec7" }}>Instant penalty:</strong> 10%
      </div>
      ...
      {unAmount > 0n && (
        <div style={{ background: "rgba(255,215,0,0.05)", border: "1px solid rgba(255,215,0,0.18)", borderRadius: 10, padding: "13px 14px", marginTop: 12 }}>
          <div style={{ fontSize: 12, color: "rgba(255,229,102,0.7)", marginBottom: 6 }}>
            Pending: <strong style={{ color: "#FFD700" }}>{fmt(unAmount, 18, 6)} MON</strong>
          </div>
          <div style={{ fontSize: 12, color: "rgba(255,229,102,0.5)", marginBottom: 10 }}>
            Ready: <strong style={{ color: canClaim ? "#39FF14" : "#ff6ec7" }}>{timeLeft(unAt)}</strong>
          </div>
      ...
      <button className="btn-outline" style={{ borderColor: "rgba(255,110,199,0.35)", color: "#ff6ec7" }} onClick={() => inAmt && tx("instantUnstake", [toWei(inAmt)])} disabled={!inAmt}>
        INSTANT UNSTAKE
      </button>

      <TxStatus s={s} />
      <div style={{ marginTop: 14, padding: "9px 12px", background: "rgba(0,0,0,0.3)", borderRadius: 8, fontSize: 9, color: "rgba(255,215,0,0.25)", fontFamily: "monospace", wordBreak: "break-all" }}>
...
function SwapTab({ account }) {
  ...
  return (
    <>
      <div style={{ fontSize: 13, color: "rgba(255,229,102,0.6)", marginBottom: 16, lineHeight: 1.8 }}>
        Powered by <strong style={{ color: "#FFD700" }}>NEAR Intents</strong> — swap ETH, BTC, SOL, USDC to DRAGON
      </div>
      <div className="field">
        ...
        <>
          ...
          {quote.depositAddress && (
            <div className="deposit-box">
              <div style={{ color: "#FFD700", marginBottom: 6, fontFamily: "'Cinzel',serif", fontWeight: 700 }}>DEPOSIT ADDRESS:</div>
              {quote.depositAddress}
...
function AirdropTab() {
  return (
    <>
      <div style={{ background: "linear-gradient(135deg,rgba(255,215,0,0.08),rgba(57,255,20,0.05))", border: "1px solid rgba(255,215,0,0.22)", borderRadius: 14, padding: "22px 18px", marginBottom: 18, textAlign: "center" }}>
        <div style={{ fontSize: 44, marginBottom: 10 }}>🏆</div>
        <div style={{ fontFamily: "'Cinzel',serif", fontSize: 15, fontWeight: 700, color: "#FFD700", letterSpacing: 2 }}>BUY DRAGON · WIN GOLD</div>
        <div style={{ fontSize: 12, color: "rgba(255,229,102,0.6)", marginTop: 10, lineHeight: 1.8 }}>
          Stake MON → earn 1 GOLD + 1 DRAGON per MON per day.<br />
          GOLD is the new reward token, verified on Sourcify.
        </div>
      </div>
      ...
      {[
        ...
        ["DRAGON Token",         DRAGON_ADDRESS,  "#FFE566"],
        ["LiquidStaking",        STAKING_ADDRESS, "#FFE566"],
      ].map(([l, a, c]) => (
      ...
      <div style={{ background: "rgba(57,255,20,0.05)", border: "1px solid rgba(57,255,20,0.18)", borderRadius: 10, padding: "12px 14px", marginTop: 16, fontSize: 12, color: "rgba(255,229,102,0.7)", lineHeight: 1.9 }}>
        <strong style={{ color: "#39FF14" }}>How it works:</strong><br />
        1. Stake MON in the Stake tab<br />
        2. Earn 1 GOLD + 1 DRAGON / MON / day<br />
        3. Claim rewards anytime from the Stake tab<br />
        4. GOLD and DRAGON go directly to your wallet
      </div>
...
function InfoTab() {
  ...
  const addrs = [
    ...
    ["LiquidStaking",    STAKING_ADDRESS, "Stake MON · Earn GOLD+DRAGON"],
  ...
  return (
    <>
      ...
      {[
        ...
        ["Chain ID",       "143",                       false],
        ["Rewards/day",    "1 GOLD + 1 DRAGON per MON", true ],
        ...
        <div key={k} className="info-row">
          ...
          <span className="v" style={hi ? { color: "#39FF14" } : {}}>{v}</span>
        </div>
      ))}

      <div className="card-title" style={{ marginTop: 20 }}>ADDRESSES</div>
      {addrs.map(([l, a, d]) => (
        <div key={l} style={{ background: "rgba(10,8,0,0.9)", border: "1px solid rgba(255,215,0,0.14)", borderRadius: 10, padding: "11px 13px", marginBottom: 7, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontFamily: "'Cinzel',serif", fontSize: 8, color: "rgba(255,215,0,0.45)", letterSpacing: 2, textTransform: "uppercase" }}>{l}</div>
            <div style={{ fontFamily: "monospace", fontSize: 10, color: "#FFE566", marginTop: 2 }}>{a}</div>
            <div style={{ fontSize: 9, color: "rgba(255,229,102,0.35)", marginTop: 2 }}>{d}</div>
          </div>
          <button onClick={() => copy(a, l)} style={{ padding: "4px 10px", borderRadius: 6, cursor: "pointer", border: "1px solid rgba(255,215,0,0.3)", background: cop === l ? "rgba(255,215,0,0.18)" : "rgba(255,215,0,0.05)", color: cop === l ? "#FFD700" : "rgba(255,215,0,0.5)", fontFamily: "'Cinzel',serif", fontSize: 8, letterSpacing: 2, whiteSpace: "nowrap" }}>
            {cop === l ? "COPIED" : "COPY"}
          </button>
        </div>
      ))}
...
function Banner() {
  ...
  const links = [
    ...
    { href: "https://x.com/bnbgold277983", label: "TWITTER",  color: "#FFD700" },
    { href: "https://discord.com/channels/1316093079090106472", label: "DISCORD", color: "#a259ff" },
    { href: MONAD_VISION, label: "ANALYTICS", color: "#FFD700" },
  ];
  return (
    <div style={{ position: "relative", zIndex: 5, padding: "0 14px 24px" }}>
      <div style={{ height: 2, background: "linear-gradient(90deg,#FFD700,#39FF14,#00eaff,#a259ff,#FFD700)", backgroundSize: "200% 100%", animation: "bbScroll 4s linear infinite", borderRadius: "2px 2px 0 0" }} />
      <div style={{ border: "1px solid rgba(255,215,0,0.18)", borderTop: "none", borderRadius: "0 0 14px 14px", overflow: "hidden", background: "linear-gradient(160deg,#050A0E,#000d00,#001a00)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 16px", borderBottom: "1px solid rgba(255,215,0,0.07)", flexWrap: "wrap" }}>
          <span style={{ fontFamily: "monospace", fontSize: 9, color: "rgba(255,215,0,0.35)" }}>DRAGON: {DRAGON_ADDRESS}</span>
          <button onClick={() => { navigator.clipboard.writeText(DRAGON_ADDRESS).then(() => { setCop(true); setTimeout(() => setCop(false), 2000); }); }} style={{ padding: "3px 9px", borderRadius: 5, cursor: "pointer", border: "1px solid rgba(255,215,0,0.28)", background: cop ? "rgba(255,215,0,0.14)" : "rgba(255,215,0,0.04)", color: cop ? "#FFD700" : "rgba(255,215,0,0.45)", fontFamily: "'Cinzel',serif", fontSize: 7, letterSpacing: 2 }}>
            {cop ? "COPIED" : "COPY"}
          </button>
        </div>
        <div style={{ display: "flex", flexWrap: "wrap" }}>
          {links.map((l, i) => (
            <a key={i} href={l.href} target="_blank" rel="noopener noreferrer" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "14px 16px", textDecoration: "none", flex: 1, minWidth: 90, borderRight: i < links.length - 1 ? "1px solid rgba(255,215,0,0.07)" : "none" }}>
              <span style={{ fontFamily: "'Cinzel',serif", fontSize: 8, fontWeight: 700, letterSpacing: 2, color: l.color }}>{l.label}</span>
            </a>
          ))}
        </div>
        <div style={{ textAlign: "center", padding: "8px 16px 12px", fontSize: 7, color: "rgba(255,215,0,0.18)", letterSpacing: 4, fontFamily: "'Cinzel',serif", borderTop: "1px solid rgba(255,215,0,0.06)" }}>
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        :root{--gold:#FFD700;--gold-l:#FFE566;--gold-d:#C8960C;--neon:#39FF14;--purple:#a259ff;--pink:#ff6ec7;--black:#050A0E;--bdr:rgba(255,215,0,0.22);--txt:#FFE566;--dim:rgba(255,229,102,0.6);--fnt:rgba(255,215,0,0.38);--grad:linear-gradient(135deg,#FFE566,#FFD700,#C8960C,#B8860B);}
        body{background:var(--black);color:var(--txt);font-family:'Rajdhani',sans-serif;}
        .app{min-height:100vh;background:radial-gradient(ellipse 80% 55% at 50% 0%,rgba(40,30,0,0.7),transparent 65%),radial-gradient(ellipse 40% 30% at 10% 80%,rgba(57,255,20,0.07),transparent 55%),var(--black);position:relative;overflow-x:hidden;}
        .app::before{content:'';position:fixed;inset:0;z-index:0;pointer-events:none;background:repeating-linear-gradient(0deg,transparent,transparent 3px,rgba(255,215,0,0.01) 3px,rgba(255,215,0,0.01) 4px);}
        .particles{position:fixed;inset:0;pointer-events:none;z-index:1;}
        .dp{position:absolute;bottom:-20px;border-radius:50%;animation:dpRise linear infinite;}
        .dp0{width:4px;height:9px;background:radial-gradient(#FFD700,transparent);opacity:.5;}
        .dp1{width:3px;height:7px;background:radial-gradient(#39FF14,transparent);opacity:.35;}
        .dp2{width:5px;height:5px;background:radial-gradient(#00eaff,transparent);opacity:.2;}
        .dp3{width:2px;height:8px;background:radial-gradient(#a259ff,transparent);opacity:.25;}
        .dp4{width:3px;height:6px;background:radial-gradient(#ff6ec7,transparent);opacity:.18;}
        ...
        @keyframes titleGlow{0%{filter:drop-shadow(0 0 18px rgba(255,215,0,.5));}100%{filter:drop-shadow(0 0 32px rgba(255,229,102,.7));}}
        @keyframes float{0%,100%{transform:translateY(0);}50%{transform:translateY(-13px);}}
        @keyframes fadeUp{from{opacity:0;transform:translateY(14px);}to{opacity:1;transform:translateY(0);}}
        @keyframes goldPulse{0%,100%{box-shadow:0 0 16px rgba(255,215,0,.35);}50%{box-shadow:0 0 30px rgba(255,215,0,.65),0 0 52px rgba(200,150,12,.3);}}
        .wrap{position:relative;z-index:2;}
        .hdr{display:flex;align-items:center;justify-content:space-between;padding:13px 28px;border-bottom:1px solid rgba(255,215,0,0.16);background:rgba(5,8,2,.97);backdrop-filter:blur(20px);}
        ...
        .badge{display:inline-flex;align-items:center;gap:5px;padding:4px 12px;border-radius:20px;background:rgba(255,215,0,.07);border:1px solid rgba(255,215,0,.22);font-size:9px;color:var(--gold);letter-spacing:2px;font-family:'Cinzel',serif;}
        ...
        .divider{margin:20px auto;width:190px;height:1px;background:linear-gradient(90deg,transparent,#FFD700,#39FF14,transparent);}
        .social-links{display:flex;justify-content:center;gap:7px;flex-wrap:wrap;margin-top:4px;}
        .slink{display:inline-flex;align-items:center;gap:5px;padding:6px 14px;border-radius:20px;background:rgba(255,215,0,.06);border:1px solid rgba(255,215,0,.18);color:var(--dim);text-decoration:none;font-size:10px;font-family:'Cinzel',serif;letter-spacing:1px;transition:all .2s;}
        .slink:hover{background:rgba(255,215,0,.12);border-color:var(--gold);color:var(--gold);}
        .stats{display:flex;justify-content:center;flex-wrap:wrap;gap:9px;padding:0 28px 28px;}
        .stat-card{background:linear-gradient(135deg,rgba(20,15,0,.92),rgba(5,10,14,.97));border:1px solid var(--bdr);border-radius:11px;padding:14px 20px;min-width:140px;text-align:center;position:relative;overflow:hidden;transition:all .3s;}
        .stat-card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#FFD700,#39FF14,transparent);}
        ...
        .stat-value{font-family:'Cinzel',serif;font-size:13px;font-weight:700;margin-top:4px;background:var(--grad);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
        .tabs{display:flex;justify-content:center;gap:5px;padding:0 14px 20px;flex-wrap:wrap;}
        .tab-btn{padding:8px 18px;border-radius:6px;border:1px solid var(--bdr);background:transparent;color:var(--dim);font-family:'Cinzel',serif;font-size:9px;letter-spacing:2px;cursor:pointer;transition:all .2s;-webkit-tap-highlight-color:transparent;}
        .tab-btn.active{background:rgba(255,215,0,.1);color:var(--gold-l);border-color:var(--gold);box-shadow:0 0 12px rgba(255,215,0,.18);}
        .tab-btn:not(.active):hover{border-color:rgba(255,215,0,.5);color:var(--txt);}
        .panel{max-width:520px;margin:0 auto;padding:0 14px 32px;}
        .card{background:linear-gradient(135deg,rgba(10,8,0,.97),rgba(5,10,14,.99));border:1px solid var(--bdr);border-radius:15px;padding:24px;box-shadow:0 8px 44px rgba(0,0,0,.8);animation:fadeUp .3s ease;position:relative;overflow:hidden;}
        .card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#FFD700,#FFE566,#39FF14,#00eaff,transparent);}
        ...
        .card-title::after{content:'';flex:1;height:1px;background:linear-gradient(90deg,rgba(255,215,0,.5),transparent);}
        .balance-display{text-align:center;padding:24px 14px;background:radial-gradient(ellipse at center,rgba(255,215,0,.07),transparent 70%);border-radius:11px;border:1px solid rgba(255,215,0,.16);margin-bottom:20px;}
        .balance-amount{font-family:'Cinzel',serif;font-size:38px;font-weight:900;background:linear-gradient(135deg,#FFE566,#FFD700,#C8960C);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
        .balance-symbol{font-size:15px;color:var(--gold);margin-left:5px;font-family:'Cinzel',serif;}
        ...
        .field input,.field select{width:100%;padding:9px 12px;border-radius:7px;background:rgba(10,8,0,.9);border:1px solid var(--bdr);color:var(--txt);font-family:'Rajdhani',sans-serif;font-size:14px;outline:none;transition:border-color .2s;}
        .field input:focus,.field select:focus{border-color:var(--gold);}
        .field select option{background:#0a0800;color:var(--txt);}
        .btn-neon{width:100%;padding:12px;border-radius:8px;border:1px solid rgba(255,215,0,.5);cursor:pointer;background:linear-gradient(135deg,rgba(255,215,0,.13),rgba(57,255,20,.06));color:var(--gold-l);font-family:'Cinzel',serif;font-size:10px;font-weight:700;letter-spacing:3px;text-transform:uppercase;animation:goldPulse 3s ease infinite;transition:transform .15s;-webkit-tap-highlight-color:transparent;}
        .btn-neon:hover:not(:disabled){box-shadow:0 0 28px rgba(255,215,0,.6);transform:scale(1.02);}
        .btn-neon:active:not(:disabled){transform:scale(0.97);}
        .btn-neon:disabled{opacity:.3;cursor:not-allowed;animation:none;}
        .btn-gold{width:100%;padding:12px;border-radius:8px;border:1px solid rgba(255,215,0,.48);cursor:pointer;background:linear-gradient(135deg,rgba(255,229,102,.15),rgba(255,215,0,.09));color:var(--gold);font-family:'Cinzel',serif;font-size:10px;font-weight:700;letter-spacing:3px;text-transform:uppercase;transition:all .2s;-webkit-tap-highlight-color:transparent;}
        .btn-gold:hover{transform:scale(1.02);box-shadow:0 0 22px rgba(255,215,0,.4);}
        .btn-outline{width:100%;padding:9px;border-radius:7px;margin-top:7px;border:1px solid rgba(255,215,0,.2);background:transparent;color:var(--dim);font-family:'Cinzel',serif;font-size:9px;cursor:pointer;transition:all .2s;letter-spacing:2px;-webkit-tap-highlight-color:transparent;}
        .btn-outline:hover{border-color:var(--gold);color:var(--gold-l);background:rgba(255,215,0,.04);}
        ...
        .status.pending{background:rgba(255,215,0,.07);color:var(--gold);border:1px solid rgba(255,215,0,.28);}
        ...
        .quote-box{margin-top:11px;padding:12px;border-radius:9px;background:rgba(255,215,0,.04);border:1px solid rgba(255,215,0,.16);}
        .quote-row{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(255,215,0,.06);font-size:12px;}
        .quote-row:last-child{border-bottom:none;}
        .quote-row span:first-child{color:var(--dim);}
        .quote-row span:last-child{color:var(--gold-l);font-weight:700;font-family:'Cinzel',serif;font-size:10px;}
        .deposit-box{margin-top:11px;padding:12px;border-radius:7px;background:rgba(255,215,0,.05);border:1px solid rgba(255,215,0,.26);word-break:break-all;font-size:10px;color:var(--gold-l);font-family:monospace;line-height:1.7;}
        .info-row{display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid rgba(255,215,0,.06);font-size:12px;}
        .info-row:last-child{border-bottom:none;}
        .k{color:var(--fnt);font-family:'Cinzel',serif;font-size:7px;letter-spacing:2px;text-transform:uppercase;}
        .v{color:var(--gold-l);font-weight:600;font-family:monospace;word-break:break-all;text-align:right;max-width:62%;font-size:10px;}
